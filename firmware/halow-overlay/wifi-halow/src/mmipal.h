...
/* Path C addition: forward-declare lwIP's netif struct (we don't want to
 * pull lwip/netif.h into every TU that just needs the pointer) and expose
 * the HaLow netif. See mmipal_lwip.c. */
struct netif;
