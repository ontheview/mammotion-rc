...
int HalowServer::setTimeout(uint32_t seconds){
  struct timeval tv;
  tv.tv_sec = seconds;
  tv.tv_usec = 0;
...
HalowClient HalowServer::available(){
  return accept();
}

HalowClient HalowServer::accept(){
  if(!_listening)
    return HalowClient();
  int client_sock;
  ...
  if(client_sock >= 0){
    ...
    if(setsockopt(client_sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&val, sizeof(int)) == ESP_OK) {
      ...
      if(setsockopt(client_sock, IPPROTO_TCP, TCP_NODELAY, (char*)&val, sizeof(int)) == ESP_OK)
        return HalowClient(client_sock);
    }
  }
  return HalowClient();
}

void HalowServer::begin(uint16_t port){
    begin(port, 1);
}

void HalowServer::begin(uint16_t port, int enable){
  if(_listening)
    return;
  if(port){
      _port = port;
...
void HalowServer::setNoDelay(bool nodelay) {
    _noDelay = nodelay;
}

bool HalowServer::getNoDelay() {
    return _noDelay;
}

bool HalowServer::hasClient() {
    if (_accepted_sockfd >= 0) {
      return true;
    }
    struct sockaddr_in6 _client;
...
void HalowServer::close(){
  end();
}

void HalowServer::stop(){
  end();
}
