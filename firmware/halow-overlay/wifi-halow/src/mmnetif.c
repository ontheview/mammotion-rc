...
err_t mmnetif_init(struct netif *netif)
{
    ...
    /* Path C diagnostic: print struct netif layout as seen by THIS TU
     * (the wifi-halow library's compile context).  Should match the print
     * from halow_init.cpp — if it doesn't, PIO is applying different build
     * flags to library files vs project files. */
    printf("[mmnetif] sizeof(struct netif)=%u  offsets: linkoutput=%u state=%u "
           "flags=%u hwaddr=%u hwaddr_len=%u name=%u  "
           "LWIP_DHCP=%d AUTOIP=%d IGMP=%d IPV6=%d "
           "STATUS_CB=%d LINK_CB=%d REMOVE_CB=%d HOSTNAME=%d "
           "NUM_CLIENT_DATA=%d\n",
           ...
           (unsigned)offsetof(struct netif, hwaddr_len),
           (unsigned)offsetof(struct netif, name),
           (int)LWIP_DHCP, (int)LWIP_AUTOIP, (int)LWIP_IGMP, (int)LWIP_IPV6,
           (int)LWIP_NETIF_STATUS_CALLBACK,
           (int)LWIP_NETIF_LINK_CALLBACK,
           (int)LWIP_NETIF_REMOVE_CALLBACK,
           (int)LWIP_NETIF_HOSTNAME,
           (int)LWIP_NUM_NETIF_CLIENT_DATA);
