...
enum mmwlan_security_type
{
    ...
    MMWLAN_SAE,
    /** Path C: catch-all used by HalowScan.cpp for unrecognised security
     *  types. Heltec's mmwlan.h had this; 2.8.2's mmwlan.h dropped it.
     *  Numeric value doesn't matter — only used as a placeholder. */
    MMWLAN_OTHER,
};

/* Path C: Heltec's mmwlan.h had `typedef enum mmwlan_security_type
 * mmwlan_security_type_t;` — the Arduino wrappers (HalowSTA.h, HalowScan.h)
 * use the _t alias.  2.8.2's mmwlan.h dropped the typedef, so we re-add it
 * here to keep the wrappers source-compatible. */
typedef enum mmwlan_security_type mmwlan_security_type_t;
