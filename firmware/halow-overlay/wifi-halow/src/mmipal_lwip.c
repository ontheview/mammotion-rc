...
/* Path C addition: expose the HaLow lwIP netif so external code (our softAP
 * NAPT setup, rate limiter, non-blocking linkoutput) can attach to it. Heltec
 * exposed `mmipal_data` via the public mmnetif.h header; mm-iot-esp32 keeps
 * mmipal_data static, so we provide a narrow accessor here. */
